...
ZZ = ((1 + YY) .* (((gama*gA+gN+rhho) ./ (1 - XX)) + delR)) / lmax;

figure;
surf(XX, YY, ZZ, 'EdgeColor','none','FaceColor','magenta'); % Semi-transparent surface
hold on;

% Fill the space below the surface
for k = 1:size(ZZ, 2)
    % For each slice in x, create a "vertical patch"
    fill3([XX(:,k); flipud(XX(:,k))], [YY(:,k); flipud(YY(:,k))], ...
          [zeros(size(ZZ(:,k))); flipud(ZZ(:,k))], ...
          'cyan', 'FaceAlpha', 0.6, 'EdgeColor', 'none');
end

zlim([0 1])
% Fill the area below the surface
%Z_fill = min(ZZ(:)) * ones(size(ZZ)); % Minimum z value
%surf(XX, YY, Z_fill, 'FaceColor', 'cyan', 'EdgeColor', 'none', 'FaceAlpha', 0.5);

text(0,0.9,0.9,'No automation','FontSize',12,'FontWeight','bold','Color','blue')
xa = [.2 .25];
ya = [.35 .65];
annotation('arrow',xa,ya,'LineWidth',2,'Color','cyan');

xlabel('Robot tax $\tau_R$','Interpreter','latex')
ylabel('Capital income tax $\tau_K$','Interpreter','latex')
zlabel('Minimum wage $\tilde{w}_{\ell}$','Interpreter','latex');
